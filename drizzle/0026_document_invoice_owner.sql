...
-- same incremental way `0011_approval_constraints.sql` added `approval` and
-- `0025_expense_and_clause_note_constraints.sql` added `expense`: DROP/ADD
-- the CHECK (metadata-only, touches no existing row) and CREATE OR REPLACE
-- the validation trigger with one more branch.
...
ALTER TABLE "document" ADD CONSTRAINT "document_owner_type_known"
	CHECK (owner_type IN ('contract', 'approval', 'expense', 'invoice'));
