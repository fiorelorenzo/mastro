...
/** `T`, unless it is a `LegalText`, in which case nothing is. */
type Forbidden<T> = T extends LegalText ? never : T;
 ...
 * compile time to accept a `LegalText` in any input slot, because a legal
 ...
 * `LegalText.svelte` instead, never through here.
 *
 ...
 * `LegalText` would satisfy it silently. Checking each supplied value
