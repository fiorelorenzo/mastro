...
test('acknowledging a run stops it looking like the newest failure', async () => {
	await inRolledBackTransaction(async (tx) => {
		...
		await tx.update(backupRun).set({ acknowledgedAt: new Date() }).where(eq(backupRun.id, row.id));

		const [updated] = await tx.select().from(backupRun).where(eq(backupRun.id, row.id));
		expect(updated.acknowledgedAt).not.toBeNull();
		// Not `toBeGreaterThan`: the trigger writes `now()`, which is the
		// transaction's own start time, so the two stamps are equal here by
		// definition.
