...
test('the risk state is a queryable, timestamped event #74 can poll: the log shows it the same day', async () => {
	await inRolledBackTransaction(async (tx) => {
		...
		// `before` is JavaScript's clock, taken after the transaction opened;
		// the row's stamp is the transaction's own `now()`, which is earlier.
		// What this test is really about is that the event is queryable and
		// dated today, so it asserts that and nothing it cannot know.
