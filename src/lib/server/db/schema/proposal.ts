...
import { check, jsonb, numeric, pgEnum, pgTable, text, timestamp, uuid } from 'drizzle-orm/pg-core';
 ...
 * a metadata-only migration, never an `ALTER TYPE ... ADD VALUE`.
 * `'contract'` (#86) and `'invoice'` (#87) were added together, in one
 * migration, because #87's own accept dispatcher depends on the same
 * widened CHECK #86 needed for `contract_id` to become nullable — see
 * `repositories/proposal.ts`'s `applyProposal` for each target's own
 * dispatcher.
 */
export type ProposalTargetType = 'work_unit' | 'contract' | 'invoice';
 ...
 * way `document.contractId` always does, for every target type except
 * `'contract'` itself (#86): a producer proposing a brand new contract has
 * no contract row yet to point at — that row is what accepting the
 * proposal creates — so this column is nullable, and the accompanying
 * custom migration's CHECK enforces the narrower rule that actually holds:
 * null only when `targetType = 'contract'`, required for everything else.
 * A `'contract'` proposal for an *existing* contract (an addendum, a rate
 * change read off a later PDF) still sets it; only first intake does not.
 *
...
export const proposal = pgTable(
	...
	(table) => [
		check(
			'proposal_contract_id_required_unless_first_intake_contract',
			sql`${table.targetType} = 'contract' or ${table.contractId} is not null`
		)
	]
);
