...
import { createWorkUnit, linkApprovalToWorkUnit } from '$lib/server/repositories/work-unit';
import {
...
test('fetchLatestBackupRun returns the most recent row, null when none exist', async () => {
	await expect(
		db.transaction(async (tx) => {
			...
			await tx.insert(backupRun).values({ status: 'failure', detail: 'first' });
			const [second] = await tx
				.insert(backupRun)
				.values({ status: 'success', detail: null })
				.returning();
...
test('fetchMirrorFailureRows excludes an already-mirrored document and attaches the latest run for one still pending', async () => {
	await expect(
		db.transaction(async (tx) => {
			...
			await tx
				.update(document)
				.set({ remoteFileId: 'remote-1' })
				.where(eq(document.id, mirrored.id));

			await tx
				.insert(documentMirrorRun)
				.values({ documentId: pending.id, status: 'failure', detail: 'first attempt' });
			...
			expect(rows).toHaveLength(1);
			expect(rows[0].documentId).toBe(pending.id);
