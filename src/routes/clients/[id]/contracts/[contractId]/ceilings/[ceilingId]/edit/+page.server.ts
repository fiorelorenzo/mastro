...
import { getCeiling, updateCeiling } from '$lib/server/repositories/ceiling';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const ceiling = await getCeiling(params.ceilingId);
	if (!ceiling || ceiling.contractId !== params.contractId) {
		error(404, m.contract_ceiling_not_found());
	}

	const crumbs = contractCrumbs(contract);
	return { contract, ceiling, crumbs };
};
...
export const actions: Actions = {
	default: async ({ request, params }) => {
		...
		try {
			await updateCeiling(params.ceilingId, result.input);
		} catch (err) {
