<script lang="ts">
	...
	import {
		...
		rateUnitLabel
	} from './contracts/[contractId=uuid]/rate-cards/rate-card-enums';
...
>
	...
	{/if}
	{#snippet actions()}
		<a href={resolve('/clients/[id=uuid]/edit', { id: data.client.id })} class="underline"
	...
	<Section title={m.client_financial_summary_heading()}>
		{#if !data.hasContract}
			...
			>
				{#snippet actions()}
					<a
						href={resolve('/clients/[id=uuid]/contracts/new', { id: data.client.id })}
						class="underline">{m.contract_new_link()}</a
					>
				{/snippet}
			</EmptyState>
		{:else}
