...
<script lang="ts">
	...
	} from '../../clients/[id=uuid]/contracts/[contractId=uuid]/rate-cards/rate-card-enums';
			...
			{:else if data.proposal.targetType === 'invoice' && data.invoiceFields}
				...
				<div class="card fields-form">
					...
					{:else if data.proposal.status === 'accepted' && data.proposal.resultId}
						<Button
							href={resolve('/invoices/[id=uuid]', { id: data.proposal.resultId })}
							variant="primary"
						>
			...
			{:else}
				<div class="card decided-fields">
					...
					{#if data.proposal.status === 'accepted' && data.proposal.resultId}
						<Button
							href={resolve('/day/[id=uuid]', { id: data.proposal.resultId })}
							variant="primary"
						>
			...
			<div class="siblings-nav">
				<Button
					href={data.siblings.previous
						? resolve('/proposals/[id=uuid]', { id: data.siblings.previous.id })
						: undefined}
					variant="secondary"
					size="sm"
				...
				<Button
					href={data.siblings.next
						? resolve('/proposals/[id=uuid]', { id: data.siblings.next.id })
						: undefined}
					variant="secondary"
					size="sm"
